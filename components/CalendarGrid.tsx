...
export default function CalendarGrid({
  ...
  return (
    <div className="grid grid-cols-[repeat(7,minmax(0,1fr))_auto] gap-2.5">
      {WEEKDAY_LABELS.map((d) => (
        <div key={d} className="text-base text-base-muted text-center pb-1.5">
          {d}
        </div>
      ))}
      <div className="text-base text-base-muted text-center pb-1.5 px-1">Weekly PnL</div>
          ...
          <Fragment key={weekIdx}>
            {week.map((day) => {
              ...
              return (
                ...
                >
                  <span className="absolute top-1.5 right-2 text-lg sm:text-2xl font-bold leading-none text-base-text">
                    {format(day, "d")}
                  </span>
                  {stat && (
                    <span className="absolute bottom-1.5 left-2 text-[11px] sm:text-sm text-base-muted leading-tight">
            ...
            >
              ...
              >
                {weekTotal < 0 ? "-" : ""}${formatMoney(weekTotal)}
              </span>
            </div>
          </Fragment>
        );
